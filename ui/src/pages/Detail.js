...
const Detail = () => {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            ...
            const findMovie = res?.data?.allRatings?.find((item) => item?.slug === slug);
                ...
                <ol className="inline-flex flex-wrap items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div className="flex items-center">
                            ...
                            <span className="ml-1 text-[1.4rem] font-normal text-white md:ml-2">{movie?.name}</span>
                        </div>
                    </li>
                </ol>
            </nav>
                    ...
                    <div className="bg-[#242525] p-4">
                        <div className="md:flex gap-10">
                            <div className="flex flex-col">
                                <div className="w-[210px] h-[310px] self-center md:self-start">
                                    <img
                                ...
                                <a
                                    href={`/phim/${movie?.slug}/${fstEp?.slug}`}
                                    className="block w-full md:w-[210px] px-5 py-3 text-center text-[1.5rem] bg-[#ffb700] rounded-md mt-4"
                                >
                                    XEM PHIM
                                </a>
                            </div>
                            <div className="flex-1 text-[#f1f1f1]">
                                <h1 className="font-bold text-[2.4rem] text-[#ffb700]">{movie?.name?.toUpperCase()}</h1>
                                <h2 className="text-[1.8rem] font-normal">{`${movie?.origin_name} (${movie?.year})`}</h2>
                                <ul className="mt-6 text-[1.6rem] font-normal">
                                    <li className="mb-2">
                                        Thể loại: <span>{movie?.category?.map((c) => c.name)?.join(', ')}</span>
                                    </li>
                                    <li className="mb-2">
                                        Quốc gia: <span>{movie?.country?.map((c) => c.name)?.join(', ')}</span>
                                    </li>
                                    <li className="mb-2">
                                        Trạng thái: <span>{movie?.episode_current + ' ' + movie?.lang}</span>
                                    </li>
                                    <li className="mb-2">
                                        Đạo diễn: <span>{movie?.director?.join(', ')}</span>
                                    </li>
                                    <li className="mb-2">
                                        Diễn viên: <span>{movie?.actor?.join(', ')}</span>
                                    </li>
                                </ul>
                                <div className="flex flex-col items-center bg-[#333333] py-2">
                                    ...
                                    <p className="font-normal text-[#f1f1f1]">
                                        {`(${divideAndRound().toFixed(1)} điểm / ${
                                            ratingMovie?.rating?.totalTurns || 1
                                        } lượt)`}
                                    </p>
                                </div>
                            </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <a
                                                    ...
                                                    className="block p-3.5 text-[1.2rem]"
                                                >
                                                    {sd?.name}
                                                </a>
                                            </li>
                        ...
                        <div className="font-normal">
                            <h2 className="text-[2.4rem] mb-5">NỘI DUNG PHIM</h2>
                        ...
                        <div className="mt-5">
                            ...
                            <div
                                ...
                                data-href={`${process.env.REACT_APP_BASE_URL}/phim/${slug}`}
                                data-width="100%"
                                data-numposts="5"
                            ></div>
                        </div>
